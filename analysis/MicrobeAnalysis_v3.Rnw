...
There appears to be little to no evidence for Oxygen metabolism for a shift point in time in which the trait started evolving differently. Now we are going to see if 4 of the "oxygen-related-traits" together have a signal of either the great oxygenization event or the secondary oxygenization event. These traits are nitrogen reduction (NitRed), oxygen metabolism (O2Met), sulfur oxidation (SulfOx) and sulfide oxidation (SulfideOx).
...
ylim.min=-270
plot(seq(0, max(branching.times(tr)),length.out=100), lik.prof,type="n", ylim=c(ylim.min, -250),xaxt="n",ylab="Log Likelihood", xlab="Time before present (by)", main="Oxygen metabolism")
...
text(grO2event, ylim.min, "Great Oxidation Event",cex=0.7)
#abline(v=secO2event,lty=2,lwd=2)
text(secO2event, ylim.min, "Secondary Oxidation Event",cex=0.7)
