...
starts2 <- readRDS("../output/newstarts2.rds")
starts2 <- lapply(starts2, as.data.frame)
names(starts2) <- names(starts)
...
for(i in 1:length(Fns)){
  fns <- Fns[[i]]
  profs[[i]] <- profiles(1, fns$ARD.R2, fns$tdList, starts2, res=NULL, seq=seq(0.1, TLs[[i]], length.out=37), cores=10, start4=TRUE) 
}
...
saveRDS(profs, "../output/profs_20youngTrees_starts2.rds")
profs <- readRDS("../output/profs_20youngTrees_starts2.rds")
