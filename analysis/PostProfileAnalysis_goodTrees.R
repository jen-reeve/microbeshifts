...
keep <- (1:26)[-2]
cumsums <- lapply(sumprof2, function(x) sapply(x[keep], function(y) y$slnL[1:37]))
...
names(allRes) <- newnames[keep]
medAllRes <- lapply(allRes, function(x) apply(x,1, median))
cumsumAll <- apply(do.call(rbind, medAllRes), 2, sum)
...
sumRes <- lapply(allRes, function(x) rev(apply(x, 1, median)))
...
Rs <- do.call(rbind, lapply(sumprof, function(x) do.call(rbind, x[keep])))
...
for(i in o){
  ...
  maxCS <- max(cs)
  rs <- apply(t(sapply(sumprof, function(x) unlist(x[keep][[i]][maxIndex,2:3]))), 2, median)
  lwds <- abs(log(rs))*1.2+0.5
  cols <- pal(100)[floor(97*(log(rs)+7)/17)+1]
  plot(allcstime, cs, ylim=c(0, max(cs)+0.2*max(cs)), xlim=c(2.6,0),xaxt="n",xlab="",ylab="", yaxt="n", type="n", main=gsub("_", " ", newnames[keep][i]), cex.main=2.5)
